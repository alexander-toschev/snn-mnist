...
LOG="$OUTDIR/${DATASET//[: ,]/_}_rgb32_T300_N5000_2layer_greedy_localinhib_s085_rate00233_${STAMP}.log"
mkdir -p "$OUTDIR"

echo "Run ${DATASET} RGB32 (diverse10) T=300 N=5000 2layer greedy + local_inhib(s=0.85) rate_scale=0.00233 started at $(date -Is)" | tee -a "$LOG"
...
python agent_mvp/experiment_runner.py \
  ...
  --set encoder=poisson --set poisson_deterministic=true --set poisson_rate_scale=0.00233 --set encoder_rate_boost=1.0 \
  --set time=300 --set N=5000 \
