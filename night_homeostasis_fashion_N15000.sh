...
LOG="$OUTDIR/homeo_fashion_N15000_${STAMP}.log"
SUMMARY_JSONL="$OUTDIR/homeo_fashion_N15000_${STAMP}.summary.jsonl"

mkdir -p "$OUTDIR"

echo "Homeostasis N=15000 started at $(date -Is)" | tee -a "$LOG"
...
# Best config from N=5000 sweep (+ baseline for comparison)
...
echo -e "\nHomeostasis N=15000 finished at $(date -Is)" | tee -a "$LOG"
