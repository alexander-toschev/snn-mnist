...
# 10 intentionally diverse CIFAR-100 fine labels (torchvision indexing)
DATASET="cifar100:0,1,2,3,5,8,13,14,17,19"

OUTDIR="runs_csnn"
STAMP=$(date -u +%Y%m%dT%H%M%SZ)
LOG="$OUTDIR/${DATASET//[: ,]/_}_rgb32_T200_N5000_3layer_greedy_localinhib_s085_rate007_${STAMP}.log"
mkdir -p "$OUTDIR"

echo "Run ${DATASET} RGB32 (diverse10) T=200 N=5000 3layer greedy + local_inhib(s=0.85) rate_scale=0.007 started at $(date -Is)" | tee -a "$LOG"
...
python agent_mvp/experiment_runner.py \
  ...
  --set c3_out=128 --set c3_kernel=3 --set c3_pad=1 --set c3_stride=2 \
  --set greedy_enable=true --set greedy_n1=2500 --set greedy_n2=2500 \
  --set encoder=poisson --set poisson_deterministic=true --set poisson_rate_scale=0.007 --set encoder_rate_boost=1.0 \
  --set time=200 --set N=5000 \
